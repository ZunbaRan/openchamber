...
- New packages embed the publisher public key inside the signed index. OpenChamber first verifies that the package is internally consistent, then shows publisher identity, key id, fingerprint, Tool/Skill inventory, network permissions, and Native-code status for explicit confirmation. The embedded key does not prove real-world identity; compare its fingerprint through an independent channel for production.
