...
Plus everything from the shared OpenLoop UI: branchable timeline, smart tool UIs, voice mode, Git workflows, and more.

## Commands

| Command | Description |
|---------|-------------|
| `OpenLoop: Focus Chat` | Focus the chat panel |
| `OpenLoop: New Session` | Start a new chat session |
| `OpenLoop: Open Sidebar` | Open the OpenLoop sidebar |
| `OpenLoop: Open Agent Manager` | Launch parallel multi-model runs |
| `OpenLoop: Open Session in Editor` | Open current or new session in an editor tab |
| `OpenLoop: Settings` | Open extension settings |
| `OpenLoop: Restart API Connection` | Restart the OpenCode API process |
| `OpenLoop: Show OpenCode Status` | Debug info for development or bug reports |

### Right-click menu

Select code in the editor, right-click, and find the **OpenLoop** submenu:
