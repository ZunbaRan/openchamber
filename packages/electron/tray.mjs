...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'OpenLoop — no active sessions';
  const bits = [];
  ...
  return `OpenLoop — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    if (isLinux || process.platform === 'win32') {
      ...
      template.push(
        { label: 'Show OpenLoop', click: () => onAction({ type: 'show-main-window' }) },
        { type: 'separator' },
        { label: 'Quit OpenLoop', click: () => onAction({ type: 'quit' }) },
      );
    }
