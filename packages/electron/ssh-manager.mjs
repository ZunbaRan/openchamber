...
const waitLocalForwardReady = async (localPort) => {
  ...
  throw new Error('Timed out waiting for forwarded OpenLoop health');
};
...
export class ElectronSshManager {
  ...
  async issueClientToken(localUrl, openchamberPassword) {
    ...
    const loginResponse = await fetch(new URL('/auth/session', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({
        ...
        clientLabel: 'OpenLoop Desktop SSH',
      }),
    });
    if (!loginResponse.ok) {
      throw new Error(`Configured OpenLoop UI password was rejected by forwarded server (status ${loginResponse.status})`);
    }
    ...
    const tokenResponse = await fetch(new URL('/api/client-auth/clients', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({ label: 'OpenLoop Desktop SSH' }),
    });
  ...
  async installOpenChamberManaged(parsed, controlPath, version, preferred) {
    ...
    throw lastError || new Error('Failed to install OpenLoop on remote host');
  }
    ...
    if (isLivenessHttpStatus(infoStatus)) {
      if (isAuthHttpStatus(infoStatus)) {
        if (openchamberPassword && authStatus !== 200) {
          throw new Error(`Remote OpenLoop requires UI authentication and configured password was rejected (auth status ${authStatus})`);
        }
        if (isLivenessHttpStatus(healthStatus)) return {};
        throw new Error('Remote OpenLoop requires UI authentication on /api/system/info; configure OpenLoop UI password');
      }
    } else if (isLivenessHttpStatus(healthStatus)) {
      return {};
    } else {
      throw new Error(`Remote OpenLoop probe failed (info status ${infoStatus}, health status ${healthStatus})`);
    }
  ...
  async ensureRemoteServer(instance, parsed, controlPath) {
    if (instance.remoteOpenchamber.mode === 'external') {
      if (!instance.remoteOpenchamber.preferredPort) {
        throw new Error('External mode requires a preferred remote OpenLoop port');
      }
      const port = instance.remoteOpenchamber.preferredPort;
      this.setStatus(instance.id, 'server_detecting', 'Probing external OpenLoop server', null, null, port, false, 0, false);
    ...
    this.setStatus(instance.id, 'remote_probe', 'Checking remote OpenLoop installation');
    const installedVersion = await this.currentRemoteOpenChamberVersion(parsed, controlPath);
    if (!installedVersion) {
      this.setStatus(instance.id, 'installing', 'Installing OpenLoop on remote host');
      ...
      this.setStatus(instance.id, 'updating', `Updating remote OpenLoop from ${installedVersion} to ${this.appVersion}`);
      await this.installOpenChamberManaged(parsed, controlPath, this.appVersion, instance.remoteOpenchamber.installMethod);
    }

    this.setStatus(instance.id, 'server_detecting', 'Detecting managed OpenLoop server');
    ...
    if (!remotePort) {
      this.setStatus(instance.id, 'server_starting', 'Starting managed OpenLoop server');
    ...
    if (!(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredOpenChamberPassword(instance)))) {
      throw new Error('Managed OpenLoop server failed to become reachable');
    }
