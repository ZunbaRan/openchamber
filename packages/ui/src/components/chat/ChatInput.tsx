...
import {
  ReviewFlowDialog,
  type ReviewFlowExecution,
} from "@/components/session/ReviewFlowDialog";
import {
  AttachedFilesList,
...
const renameFileForAttachmentCitation = (
  file: File,
  filename: string,
): File => {
...
const getFileMentionInputSourceForInsertedText = (
  insertedText: string,
): FileMentionAutocompleteInputSource =>
 ...
 */
const collectInlineSkillMentions = (
  text: string,
  skillNames: Set<string>,
): string[] => collectKnownTokenNames(text, "/", skillNames, "exact");
...
const ChatInputComponent: React.FC<ChatInputProps> = ({
  ...
  const initialDraftSnapshotRef = React.useRef<ChatDraftSnapshot>({
    text: "",
  ...
  const confirmedMentionsRef = React.useRef<Set<string>>(
    initialDraftSnapshotRef.current.confirmedMentions,
  );
  ...
  const [mobileDraftPicker, setMobileDraftPicker] = React.useState<
    "project" | "branch" | null
  >(null);
  const [mobileDraftPickerQuery, setMobileDraftPickerQuery] =
    React.useState("");
  ...
  const composerViewStore = React.useRef(
    createComposerEditorViewStore(),
  ).current;
  React.useEffect(
    () => () => {
  ...
  const suppressNextFileMentionPasteTimeoutRef = React.useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  const currentChatDraftIdentityRef = React.useRef<ChatDraftIdentity | null>(
    initialDraftIdentityRef.current,
  );
  ...
  const draftPermissionAutoAcceptEnabled = useSessionUIStore((s) =>
    s.newSessionDraft?.open
      ? s.newSessionDraft.permissionAutoAcceptEnabled === true
      : false,
  );
  const setNewSessionDraftTarget = useSessionUIStore(
    (s) => s.setNewSessionDraftTarget,
  );
  const setDraftPermissionAutoAcceptEnabled = useSessionUIStore(
    (s) => s.setDraftPermissionAutoAcceptEnabled,
  );
  ...
  const attachmentCompatibilityRef = React.useRef({
    ...
    modalitySignature:
      currentModelMetadata?.modalities?.input?.slice().sort().join(",") ?? null,
  ...
  React.useEffect(() => {
    ...
    const metadataBecameAvailable =
      previous.modalitySignature === null && modalitySignature !== null;
    const filesToCheck =
      modelChanged || metadataBecameAvailable
        ? attachedFiles
    ...
    const incompatibleFiles = getUnsupportedAttachmentInputs(
      filesToCheck,
      inputModalities,
    );
    if (incompatibleFiles.length === 0) return;

    const unsupportedModalities = Array.from(
      new Set(incompatibleFiles.map(({ modality }) => modality)),
    );
    ...
    const filenames = incompatibleFiles.map(
    ...
    toast.warning(
      t("chat.chatInput.toast.unsupportedAttachmentModalities", {
        model: currentModelMetadata.name ?? currentModelId ?? "",
        modalities: unsupportedModalities
          .map((modality) => modalityLabels[modality])
          .join(", "),
        files: fileSummary,
      }),
  ...
  const handleStartReviewFlow = React.useCallback(
    async (execution: ReviewFlowExecution) => {
      ...
      try {
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : t("diffView.reviewDialog.toast.startFailed"),
        );
      } finally {
  ...
  const sanitizeAttachmentsForSend = React.useCallback(
    (files: readonly AttachedFile[] | undefined): AttachedFile[] =>
      [...(files ?? [])].map((file) => ({
        ...file,
        dataUrl:
  ...
  const inlineDraftSessionKey =
    currentSessionId ?? (newSessionDraftOpen ? "draft" : "");
  const inlineDraftDirectory =
    currentSessionDirectoryForSync ?? currentDirectory;
  ...
  const inlineDraftKey = inlineDraftTarget
    ? getInlineCommentDraftKey(
        activeRuntimeKey,
        inlineDraftTarget.directory,
        inlineDraftTarget.sessionKey,
      )
    : null;
  const draftCount = useInlineCommentDraftStore(
    React.useCallback(
      (state) =>
        inlineDraftKey ? (state.drafts[inlineDraftKey] ?? []).length : 0,
      [inlineDraftKey],
    ),
  );
  const draftSourceKey = useInlineCommentDraftStore(
    React.useCallback(
      (state) => {
        const drafts = inlineDraftKey
          ? (state.drafts[inlineDraftKey] ?? [])
          : [];
  ...
  const terminalContextDrafts =
    terminalContextCount > 0
      ? (inlineDraftKey
          ? (useInlineCommentDraftStore.getState().drafts[inlineDraftKey] ?? [])
          : []
        ).filter((draft) => draft.source === "terminal")
      : [];
  ...
  const removeReviewDrafts = React.useCallback(() => {
    ...
    for (const draft of drafts) {
      if (
        ...
        draft.source !== "pr-comment" &&
        draft.source !== "pr-check"
      ) {
        removeInlineCommentDraft(inlineDraftTarget, draft.id);
      }
    }
  ...
  const messageHistory = useMessageHistory(
    useUserMessageHistory(currentSessionId ?? ""),
  );
  ...
  const autoReviewRunning = useAutoReviewStore(
    React.useCallback(
      (state) => {
  ...
  const hasContent =
    message.trim().length > 0 || attachedFiles.length > 0 || hasDrafts;
  ...
  const handleQueueMessage = React.useCallback(() => {
    ...
    if (!inputSnapshot.hasContent || !currentSessionId || !messageQueueTarget)
      return;
  ...
  const handleSubmit = async (options?: SubmitOptions) => {
    ...
    const inputSnapshot =
      options?.presetText != null
        ? {
            ...
            hasContent:
              options.presetText.trim().length > 0 ||
              attachedFiles.length > 0 ||
              hasDrafts,
          }
        : getCurrentInputSnapshot();
    ...
    const outgoing = buildOutgoingMessage(
      {
        ...
        linkedPr: linkedPr
          ? {
              instructions: linkedPr.instructionsText,
              context: linkedPr.contextText,
            }
          : null,
      },
      {
        ...
        collectSkillNames: (text) =>
          collectInlineSkillMentions(text, availableSkillNames),
    ...
    const parsedCommand =
      inputMode === "normal" ? parseSlashCommand(primaryText) : null;
    if (parsedCommand) {
      ...
      if (command && commandIsAvailable) {
        ...
        try {
          ...
          const visibleText = await renderMagicPrompt(
            command.visiblePrompt,
            variables.visible,
          );
          const instructionsText = await renderMagicPrompt(
            command.instructionsPrompt,
          ...
          toast.error(
            error instanceof Error ? error.message : t(command.errorToastKey),
          );
        }
        return;
      }
    ...
    const restoreConsumedDrafts = () => {
      if (consumedDraftTarget && drafts.length > 0) {
        useInlineCommentDraftStore
          .getState()
          .restoreDrafts(consumedDraftTarget, drafts);
      }
    };
  ...
  const [dictationContentHeight, setDictationContentHeight] = React.useState<
    number | null
  >(null);
  const handleDictationContentHeightChange = React.useCallback(
    (height: number | null) => {
    ...
    if (isVSCodeRuntime() && suppressNextFileDropTextInsertRef.current) {
      ...
      if (
        candidateAbsolutePaths.some(
          (path) => path.length > 0 && value.includes(path),
        )
      ) {
        clearDropTextSuppression();
        return;
        ...
        if (ta && selEnd > selStart) {
          ...
          if (shouldWrapSelectionAsLink(url, selected)) {
            ...
            updateAutocompleteState(
              next,
              caret,
              getFileMentionInputSourceForInsertedText(url),
              url,
            );
            return;
          }
  ...
  const toMentionPath = React.useCallback(
    (absolutePath: string) =>
      toProjectRelativeMentionPath(absolutePath, chatSearchDirectory || ""),
    [chatSearchDirectory],
  );
  ...
  const handleVSCodePickFiles = React.useCallback(async () => {
    try {
      const data = (await vscodeApi?.pickFiles?.({
        extensions: ACCEPTED_ATTACHMENT_EXTENSIONS,
      })) as
        | {
            files?: Array<{
              name: string;
  ...
  const handleMobileNewSession = React.useCallback(() => {
    ...
    openNewSessionDraft(
      currentDirectory ? { directoryOverride: currentDirectory } : undefined,
    );
  ...
  const isPermissionAutoAcceptInteractive = Boolean(
    permissionScopeSessionId || newSessionDraftOpen,
  );
    ...
    <>
      ...
      >
        ...
        >
          ...
          {linkedPr && !isVSCode ? (
            <LinkedReferenceRow
              numberLabel={t("chat.chatInput.linked.pr.number", {
              ...
              openInBrowserLabel={t(
                "chat.chatInput.linked.pr.openInBrowserAria",
              )}
                  ...
                  >
                    ...
                    <ComposerFooter
                      ...
                      onDictationContentHeightChange={
                        handleDictationContentHeightChange
                      }
                    />
                  </div>
                </div>
      ...
      <ReviewFlowDialog
        ...
        projectDirectory={
          currentSessionDirectoryForSync ?? currentDirectory ?? null
        }
