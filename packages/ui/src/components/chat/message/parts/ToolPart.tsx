...
const HTMLArtifactView = React.lazy(() =>
  import("@/components/interactive-ui/HTMLArtifactView").then((module) => ({
    default: module.HTMLArtifactView,
  })),
);
const McpAppRenderer = React.lazy(() =>
  import("@/components/interactive-ui/McpAppRenderer").then((module) => ({
    default: module.McpAppRenderer,
  })),
);

const TOOL_ROW_TEXT_CLASS =
  "!text-[length:var(--text-meta)] !leading-5 sm:!leading-6 tracking-normal";
const TOOL_ROW_TITLE_CLASS = cn(
...
const getMultiFileDescription = (
  ...
  const entriesByPath = new Map<
    string,
    { path: string; name: string; added: number | null; removed: number | null }
  >();
...
const flushDeferredToolBodyMounts = () => {
  while (deferredToolBodyMounts.length > 0) {
    ...
    if (item.active) {
      item.fn();
      deferredToolBodyFrame =
        deferredToolBodyMounts.length > 0
...
const extractFirstChangedLineFromDiff = (
  diffText: string,
): number | undefined => {
...
const getFirstChangedLineFromMetadata = (
  ...
  if (
    !metadata ||
    (tool !== "edit" && tool !== "multiedit" && tool !== "apply_patch")
  ) {
    return undefined;
  }

  const topLevelPatch =
    getPatchText((metadata as { patch?: unknown }).patch) ??
    getPatchText(metadata.diff);
  if (topLevelPatch) {
  ...
  const filePatch =
    getPatchText(firstFile?.patch) ?? getPatchText(firstFile?.diff);
  if (filePatch) {
...
const getPrimaryDiffFromMetadata = (
  ...
  if (files.length > 0) {
    const preferred =
      typeof preferredPath === "string" && preferredPath.length > 0
        ? preferredPath
        : undefined;
    const matched = preferred
      ? files.find((file) => {
          ...
          return (
            candidate.relativePath === preferred ||
    ...
    if (matched && typeof matched === "object") {
      const patch =
        getPatchText((matched as { patch?: unknown; diff?: unknown }).patch) ??
...
const normalizeToolDiagnostic = (value: unknown): ToolDiagnostic | null => {
  ...
  const rawLine =
    typeof start?.line === "number" && Number.isFinite(start.line)
      ? Math.max(0, Math.trunc(start.line))
      : 0;
  const rawCharacter =
...
const getPrimaryToolPath = (
  ...
  if (toolName === "edit" || toolName === "multiedit") {
    const fileDiff = isRecord(metadata?.filediff)
      ? metadata.filediff
      : undefined;
...
const getToolDiagnosticSection = (
  ...
  const absolutePath = normalizedPath.startsWith("/")
    ? normalizedPath
    : `${normalizeDisplayPath(currentDirectory)}/${normalizedPath}`.replace(
        /\/+/g,
        "/",
      );
  ...
  return {
    displayPath: normalizedPath.startsWith("/")
      ? getRelativePath(normalizedPath, currentDirectory)
      : normalizedPath,
    diagnostics: visible,
...
const getLspToolDescription = (
  ...
  const operation =
    typeof input.operation === "string" ? input.operation : "lsp";
  ...
  const displayPath = filePath
    ? getRelativePath(filePath, currentDirectory)
    : "";
  ...
  const line =
    typeof input.line === "number" && Number.isFinite(input.line)
      ? Math.trunc(input.line)
      : undefined;
  const character =
    typeof input.character === "number" && Number.isFinite(input.character)
      ? Math.trunc(input.character)
      : undefined;
  const position =
...
const ToolScrollableSection: React.FC<ToolScrollableSectionProps> = ({
  ...
  <div
    className={cn("w-full min-w-0 flex-none overflow-hidden", outerClassName)}
  >
    <ScrollShadow
      className={cn(
...
const getToolOutputLanguage = (
  ...
  return detectLanguageFromOutput(
    formatEditOutput(output, part.tool, metadata),
    part.tool,
    input,
  );
};
...
const ToolScrollableTextOutput: React.FC<{
  ...
  const jsonResult = React.useMemo(
    () => tryParseJsonOutput(renderedOutput),
    [renderedOutput],
  );
  const [jsonViewMode, setJsonViewMode] = React.useState<
    "summary" | "formatted" | "raw"
  >("summary");
  ...
  const handleJsonViewChange = React.useCallback(
    (
      view: "summary" | "formatted" | "raw",
      ...
      <div className="tool-output-surface relative p-2 rounded-xl w-full min-w-0">
        <div className="absolute right-2 top-2 z-10 flex items-center gap-1">
          ...
          <Button
            ...
            aria-label={
              copiedJson
                ? t("chat.toolPart.copiedOutput")
                : t("chat.toolPart.copyOutput")
            }
            title={
              copiedJson
                ? t("chat.toolPart.copiedOutput")
          ...
          >
            <Icon
              name={copiedJson ? "check" : "file-copy"}
  ...
  return (
    <div
      className={
        part.tool === "bash"
          ? "typography-code text-muted-foreground/90"
          : undefined
      }
    >
      <WorkerHighlightedCode
...
const getTaskSummaryEntryRenderSignature = (
...
const areTaskSummaryEntriesRenderEqual = (
  ...
  for (let index = 0; index < prevEntries.length; index += 1) {
    if (
      getTaskSummaryEntryRenderSignature(prevEntries[index]) !==
      getTaskSummaryEntryRenderSignature(nextEntries[index])
    ) {
      return false;
    }
  }
...
const TaskSummaryEntryRow = React.memo(
  ({
    ...
    const toolName =
      normalizedToolName.length > 0 ? normalizedToolName : "tool";
    const label = getTaskSummaryLabel(entry);
  ...
  (prev, next) => {
    return (
      ...
      getTaskSummaryEntryRenderSignature(prev.entry) ===
        getTaskSummaryEntryRenderSignature(next.entry)
    );
  },
);
  ...
  ({
    ...
    return (
      <ToolScrollableSection
        maxHeightClass={isExpanded ? "max-h-[40vh]" : "max-h-56"}
        disableHorizontal
      >
        <div className="w-full min-w-0 space-y-1">
          ...
          {visibleEntries.map((entry, idx) => {
            ...
            const rowKey =
              entry.id ??
              `${getTaskSummaryEntryRenderSignature(entry)}:${absoluteIndex}`;
            return (
              <TaskSummaryEntryRow
                key={rowKey}
                entry={entry}
...
const TaskToolSummary: React.FC<{
  ...
  const trimmedOutput =
    typeof output === "string" ? stripTaskMetadataFromOutput(output) : "";
    ...
    >
      ...
      {hasOutput ? (
        <div
          className={cn(
            "space-y-1",
            (entries.length > 0 || sessionId) && "pt-1",
          )}
        >
          <button
            type="button"
          ...
          >
            ...
            <span className="typography-meta text-foreground/80 font-medium">
...
const TOOL_ERROR_ICON_STYLE: React.CSSProperties = {
  color: "var(--status-error)",
};
const TOOL_NORMAL_ICON_STYLE: React.CSSProperties = {
  color: "var(--tools-icon)",
};
const TOOL_ERROR_TITLE_STYLE: React.CSSProperties = {
  color: "var(--status-error)",
};
const TOOL_NORMAL_TITLE_STYLE: React.CSSProperties = {
...
const renderPathLikeGitChanges = (path: string, grow = true) => {
  ...
  return (
    <span
      className={cn(
        "min-w-0 flex items-baseline overflow-hidden typography-ui-label",
        grow && "flex-1",
      )}
      title={path}
    >
...
const renderAnimatedPathWithIcon = (
  ...
  if (lastSlash === -1) {
    return (
      <span
        className={cn(
          "min-w-0 inline-flex items-center gap-1 overflow-hidden",
          grow && "flex-1",
        )}
        title={path}
      >
...
> {
  ...
  componentDidCatch(error: Error) {
    if (process.env.NODE_ENV === "development") {
      console.warn(
        "Tool diff preview failed; rendering raw patch instead.",
        error,
      );
    }
  }
...
const DiffPreview: React.FC<DiffPreviewProps> = React.memo(
  ({ diff, pierreTheme, pierreThemeType, diffViewMode }) => {
...
const ToolExpandedContent: React.FC<ToolExpandedContentProps> = React.memo(
  ({
    ...
    const [diffViewMode, setDiffViewMode] =
      React.useState<DiffViewMode>("unified");
    const stateWithData = state as ToolStateWithMetadata;
    const metadata = stateWithData.metadata;
    const input = stateWithData.input;
    ...
    const diffEntries = React.useMemo(
      () =>
        getDiffPatchEntries(metadata, diffContent ?? undefined, (path) =>
          getRelativePath(path, currentDirectory),
        ),
      [currentDirectory, diffContent, metadata],
    );
    const hasVisualDiffEntry = diffEntries.some(
      (entry) => entry.renderMode === "diff",
    );
    const hideToolInputPreview =
      Boolean(
        interactiveEnvelope ||
        htmlArtifactEnvelope ||
        installedArtifactEnvelope ||
        mcpAppEnvelope,
    ...
    const hasInputText =
      !hideToolInputPreview && inputTextContent.trim().length > 0;
    ...
    const renderResultContent = () => {
      ...
      const artifactEnvelope =
        htmlArtifactEnvelope ?? installedArtifactEnvelope;
      ...
      const getEntryAbsolutePath = (entry: DiffPatchEntry) =>
        entry.title.startsWith("/")
          ? entry.title
          : `${currentDirectory}/${entry.title}`.replace(/\/+/g, "/");
      const openEntryFile = (
        ...
        useUIStore
          .getState()
          .openContextFileAtLine(currentDirectory, absolutePath, line ?? 1, 1);
      };
      const openEntryDiff = (
        entry: DiffPatchEntry,
        event: React.MouseEvent<HTMLButtonElement>,
        ...
        const absolutePath = getEntryAbsolutePath(entry);
        if (runtime?.editor && runtime.runtime.isVSCode) {
          void runtime.editor.openDiff(
            "",
            absolutePath,
            `${getRelativePath(absolutePath, currentDirectory)} (changes)`,
            { line, patch: entry.patch },
          );
          return;
        }
            ...
            <div className="space-y-1">
                ...
                {diagnosticSection.diagnostics.map((diagnostic, index) => (
                  ...
                  >
                    <div className="flex items-start gap-2 min-w-0">
                      <span
                        className="typography-micro shrink-0"
                        style={{ color: "var(--status-error)" }}
                      >
              ...
              {diagnosticSection.remaining > 0 ? (
                <div className="typography-micro text-muted-foreground">
                  {t("chat.toolPart.moreErrors", {
                    count: diagnosticSection.remaining,
                  })}
                </div>
              ) : null}
            </div>
      ...
      if (part.tool === "question") {
              ...
              <div className="space-y-2">
                {parsedQA.map((qa, index) => (
                  <div key={index} className="space-y-0.5">
                    ...
                    <div className="typography-meta text-foreground whitespace-pre-wrap">
                      {qa.answer}
                    </div>
                  </div>
                ))}
        ...
        if (
          ...
          Array.isArray(questionInput.questions) &&
      ...
      if (part.tool === "task" && hasStringOutput) {
        return renderScrollableBlock(
          <div className="w-full min-w-0">
            <SimpleMarkdownRenderer
              content={coerceToText(outputString)}
              variant="tool"
              onShowPopup={onShowPopup}
            />
          </div>,
      ...
      if (part.tool === "write" && diagnosticSection) {
        return renderScrollableBlock(
          <div className="space-y-3">{renderDiagnosticsSection()}</div>,
          { className: "p-1" },
        );
      }
    ...
    if (isTodoTool) {
      ...
      const todoOutput = renderTodoOutput(
        todoContent,
        {
            ...
            {hasInputText ? (
              <div className="my-1">
                {renderScrollableBlock(
                  ...
                  {
                    maxHeightClass:
                      isWriteLikeTool && writeLikeInputPatch && isExpanded
                        ? "max-h-[50vh]"
                        : "max-h-60",
                    className:
                      part.tool === "bash"
...
const ToolPartContent: React.FC<ToolPartProps> = ({
  ...
  const inspectionEnvelope = React.useMemo(() => {
    ...
    return typeof output === "string"
      ? parseInteractiveResultEnvelope(output)
      : null;
  }, [state]);
  ...
  const [activeLatched, setActiveLatched] =
    React.useState<boolean>(!isFinalized);
  ...
  const [pinnedTime, setPinnedTime] = React.useState<{
    start?: number;
    end?: number;
  }>(() => ({
  ...
  const [localStartAt, setLocalStartAt] = React.useState<number | undefined>(
    undefined,
  );
  const [localFinalizedAt, setLocalFinalizedAt] = React.useState<
    number | undefined
  >(undefined);
  ...
  React.useEffect(() => {
    setPinnedTime((prev) => {
      ...
      if (
        ...
        (typeof prev.start !== "number" || time.start < prev.start)
      ) {
      ...
      if (
        ...
        (typeof prev.end !== "number" || time.end > prev.end)
      ) {
        next.end = time.end;
        changed = true;
      }
  ...
  const taskOutputString = React.useMemo(() => {
    return typeof stateWithData.output === "string"
  ...
  const hasFinalMetadataTaskSummary =
    isFinalized && metadataTaskSummaryEntries.length > 0;
  ...
  const taskSessionId = React.useMemo<string | undefined>(() => {
    ...
    isTaskTool,
    metadata,
    parsedTaskMetadata.sessionId,
    partMetadata,
    taskOutputString,
  ]);

  const childSessionLookupId = hasFinalMetadataTaskSummary
    ? ""
    : (taskSessionId ?? "");

  const childSessionMessages = useSessionMessageRecords(
    childSessionLookupId,
    currentDirectory,
  );
  ...
  >(() => {
    ...
    if (
      !Array.isArray(childSessionMessages) ||
  ...
  const effectiveTimeEnd = isFinalized
    ? (pinnedTime.end ?? time?.end ?? localFinalizedAt)
    : undefined;
  ...
  const taskSummaryRenderSignature = React.useMemo(() => {
    return taskSummaryEntries
      .map(getTaskSummaryEntryRenderSignature)
      .join("\u0000");
  }, [taskSummaryEntries]);
  ...
  React.useEffect(() => {
    ...
    if (
      previous === null ||
      previous === taskSummaryRenderSignature ||
      taskSummaryEntries.length === 0
    ) {
      return;
    }
  ...
  const diffStats = React.useMemo(() => {
    return normalizedPartTool === "edit" ||
  ...
  const isMultiFileApplyPatch =
    ...
    (metadata?.files as []).length > 1;
  const normalizedPart =
    normalizedPartTool !== part.tool
      ? ({ ...part, tool: normalizedPartTool } as ToolPartType)
      : part;
  const descriptionPath = getToolDescriptionPath(
    normalizedPart,
    state,
    currentDirectory,
  );
  ...
  const handleMainClick = (e: { stopPropagation: () => void }) => {
    ...
    if (typeof filePath === "string") {
      ...
      if (!filePath.startsWith("/")) {
        absolutePath = currentDirectory.endsWith("/")
      ...
      if (
        ...
        void runtime.editor.openDiff("", absolutePath, label, {
          line: targetLine,
          patch: toolDiff,
        });
        return;
  ...
  const iconStyle =
    !isTaskTool && isError ? TOOL_ERROR_ICON_STYLE : TOOL_NORMAL_ICON_STYLE;
  const titleStyle =
    !isTaskTool && isError ? TOOL_ERROR_TITLE_STYLE : TOOL_NORMAL_TITLE_STYLE;
  const shouldRenderTaskSummary = useDeferredExpandedContent(
    isTaskTool &&
      (taskSummaryEntries.length > 0 ||
        isActive ||
        shouldTreatAsFinalized ||
        !!taskSessionId),
  );
  const shouldRenderExpandedContent = useDeferredExpandedContent(
        ...
        >
          ...
          {isMultiFileApplyPatch ? (
            ...
            <>
              ...
              {normalizedPartTool === "bash" &&
              typeof effectiveTimeStart === "number" ? (
                <span
                  className={cn(
                    "flex-shrink-0 tabular-nums text-muted-foreground/80",
                    TOOL_ROW_DESCRIPTION_CLASS,
                  )}
                >
                  <LiveDuration
                    start={effectiveTimeStart}
                      ...
                      typeof effectiveTimeEnd === "number"
                        ? effectiveTimeEnd
                        : undefined
                    }
                    active={Boolean(
                      isActive && typeof effectiveTimeEnd !== "number",
                    )}
                  />
                </span>
              ) : null}
  ...
  render() {
    ...
    return (
      <div className="flex items-center gap-1.5 pr-2 pl-px py-1.5 rounded-xl min-w-0">
        <div
          className="h-3.5 w-3.5 flex-shrink-0"
          style={TOOL_ERROR_ICON_STYLE}
        >
          {getToolIcon(this.props.toolName)}
        </div>
        <span
          className={cn(TOOL_ROW_TITLE_CLASS, "flex-shrink-0")}
          style={TOOL_ERROR_TITLE_STYLE}
        >
