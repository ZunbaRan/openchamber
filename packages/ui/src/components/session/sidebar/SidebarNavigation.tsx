...
const navIconClass = 'size-[18px] shrink-0 text-muted-foreground transition-colors group-hover:text-foreground';
...
export function SidebarNavigation({
  ...
  const contextPanelState = useUIStore((state) => (
    normalizedEffectiveDirectory ? state.contextPanelByDirectory[normalizedEffectiveDirectory] : undefined
  ));
  ...
  const openSettingsPage = React.useCallback((page: string) => {
    setSettingsPage(page);
    setSettingsDialogOpen(true);
  }, [setSettingsDialogOpen, setSettingsPage]);

  const handleNavItem = React.useCallback((id: typeof SIDEBAR_PRIMARY_NAV_ITEMS[number]['id']) => {
    if (id === 'git') {
    ...
    const item = SIDEBAR_PRIMARY_NAV_ITEMS.find((candidate) => candidate.id === id);
    if (item) {
      openSettingsPage(item.target);
    }
  }, [effectiveDirectory, openContextPanelTab, openScheduledTasksDialog, openSettingsPage, setRightSidebarOpen, t]);

  const labels = React.useMemo<Record<typeof SIDEBAR_PRIMARY_NAV_ITEMS[number]['id'], string>>(() => ({
  ...
  return (
    <div className="select-none shrink-0 px-3 pb-2">
      <div className="mb-2 flex h-11 items-center gap-1">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="min-w-[220px]">
            ...
            <DropdownMenuItem onClick={onOpenAbout}>
              ...
              <span>{t('sessions.sidebar.footer.actions.aboutOpenChamber')}</span>
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <button
          ...
          className={cn(
            ...
            isSessionSearchOpen && 'bg-interactive-selection text-interactive-selection-foreground',
          )}
      ...
      <nav className="flex flex-col gap-0.5" aria-label={t('shell.navigation.primaryAria')}>
        <div className="group/new-task flex h-7 items-center rounded-lg hover:bg-interactive-hover/60 focus-within:ring-2 focus-within:ring-primary/50">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="min-w-[230px]">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={openMultiRunLauncher} disabled={!canOpenMultiRun}>
                <Icon name="git-merge" className="size-4" />
                <div className="flex min-w-0 flex-col">
                  <span>{t('shell.navigation.compareModels')}</span>
                  <span className="typography-micro text-muted-foreground">{t('shell.navigation.advanced')}</span>
                </div>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        ...
        {SIDEBAR_PRIMARY_NAV_ITEMS.map((item) => {
          const active = item.id === 'git'
            ? isRightSidebarOpen && activeContextTab?.mode === 'git'
            : item.id !== 'scheduled' && isSettingsDialogOpen && settingsPage === item.target;
          return (
            <button
              key={item.id}
              type="button"
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="min-w-[220px]">
            <DropdownMenuItem onClick={onToggleSelectionMode}>
              ...
              {selectionModeEnabled ? <Icon name="check" className="ml-auto size-4 text-primary" /> : null}
            </DropdownMenuItem>
            ...
            <DropdownMenuSub>
              ...
              <DropdownMenuSubContent className="min-w-[180px]">
                {([
                  ...
                  <DropdownMenuItem key={value} onClick={() => setProjectSortOrder(value)}>
                    <span>{t(label)}</span>
                    {projectSortOrder === value ? <Icon name="check" className="ml-auto size-4 text-primary" /> : null}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuSubContent>
            </DropdownMenuSub>
            ...
            <DropdownMenuSub>
              ...
              <DropdownMenuSubContent className="min-w-[210px]">
                <DropdownMenuItem onClick={() => setDisplayMode('default')}>
                  ...
                  {displayMode === 'default' ? <Icon name="check" className="ml-auto size-4 text-primary" /> : null}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setDisplayMode('minimal')}>
                  <span>{t('sessions.sidebar.header.displayMode.minimal')}</span>
                  {displayMode === 'minimal' ? <Icon name="check" className="ml-auto size-4 text-primary" /> : null}
                </DropdownMenuItem>
                {showRecentControls ? (
                  <>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem onClick={toggleRecentSection}>
                      ...
                      {showRecentSection ? <Icon name="check" className="ml-auto size-4 text-primary" /> : null}
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={toggleArchivedSessions}>
                      <span>{t('sessions.sidebar.header.displayMode.showArchived')}</span>
                      {showArchivedSessions ? <Icon name="check" className="ml-auto size-4 text-primary" /> : null}
                    </DropdownMenuItem>
                  </>
                ) : null}
              </DropdownMenuSubContent>
