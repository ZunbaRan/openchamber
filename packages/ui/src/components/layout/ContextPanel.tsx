...
const getPreviewConsoleFilterMatch = (
  ...
  if (filter === "errors")
    return (
      event.level === "error" ||
      event.level === "runtime" ||
      event.level === "resource"
    );
  if (filter === "warnings") return event.level === "warn";
  return (
    event.level === "log" || event.level === "info" || event.level === "debug"
  );
};
...
const getTabIcon = (tab: {
  ...
  if (tab.mode === "preview") {
    return (
      <Icon name="global" className="h-3.5 w-3.5 text-[var(--status-info)]" />
    );
  }
...
const EditorTreeColumn: React.FC<{ visible: boolean }> = ({ visible }) => {
  ...
  const clampTreeWidth = React.useCallback((value: number) => {
    return Math.min(
      ...
      Math.max(EDITOR_TREE_MIN_WIDTH, Math.round(value)),
    );
  }, []);
...
const getSessionIDFromDedupeKey = (
...
const useSessionTitleMap = (
  directory: string | undefined,
  sessionIDs: readonly string[],
): ReadonlyMap<string, string> => {
  const store = useDirectoryStore(directory);
  const snapshotRef = React.useRef<ReadonlyMap<string, string>>(
  ...
  return React.useSyncExternalStore(
    store.subscribe,
    React.useCallback(() => {
      const next = buildSessionTitleMap(
...
const runIframeScript = async <T,>(
  ...
  const evaluate = (frameWindow as Window & { eval: (code: string) => unknown })
    .eval;
...
const getPreviewProxyOrigin = (proxySrc: string): string => {
  ...
  try {
    return new URL(proxySrc || window.location.href, window.location.href)
      .origin;
  } catch {
...
const postPreviewBridgeMessage = (
  frameWindow: Window,
  proxySrc: string,
...
const PreviewPane: React.FC<PreviewPaneProps> = ({ rawUrl, onNavigate }) => {
  ...
  const [proxyRegistrationNonce, bumpProxyRegistration] = React.useReducer(
    (x: number) => x + 1,
    0,
  );
  const [proxyState, setProxyState] = React.useState<PreviewProxyState>({
    status: "idle",
  });
  ...
  const [consoleFilter, setConsoleFilter] =
    React.useState<PreviewConsoleFilter>("all");
  const [consoleEvents, setConsoleEvents] = React.useState<
    PreviewConsoleEvent[]
  >([]);
  const [inspectMode, setInspectMode] = React.useState(false);
  const [hoverTarget, setHoverTarget] =
    React.useState<PreviewElementMetadata | null>(null);
  const currentSessionId = useSessionUIStore((state) => state.currentSessionId);
  const newSessionDraftOpen = useSessionUIStore(
    (state) => state.newSessionDraft?.open,
  );
  const effectiveDirectory = useEffectiveDirectory();
  const addInlineCommentDraft = useInlineCommentDraftStore(
    (state) => state.addDraft,
  );
  ...
  const proxyCacheKey = targetKey
    ? `${getRuntimeApiBaseUrl() || "same-origin"}|${targetKey}`
    : "";
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const proxyBasePath =
          typeof body.proxyBasePath === "string" ? body.proxyBasePath : "";
        const previewToken =
        ...
        if (!proxyBasePath || !previewToken) {
          ...
          if (!cancelled) {
            setProxyState({
              status: "error",
              message: t("contextPanel.preview.proxyError"),
            });
          }
          return;
        }

        previewProxyTargetCache.set(proxyCacheKey, {
          proxyBasePath,
          previewToken,
          expiresAt,
        });
  ...
  const proxyUrlAuthKey =
    isLoopback && proxyState.status === "ready"
    ...
    urlAuthReadyKey === proxyUrlAuthKey
      ? (() => {
          ...
          return getRuntimeUrlResolver().authenticatedAsset(
            `${proxyState.proxyBasePath}${path}${search ? `?${search}` : ""}${hash}`,
          );
        })()
      : "";
    ...
    (target: PreviewElementMetadata) => {
      ...
      void (async () => {
        ...
        try {
          ...
          const screenshot = iframe
            ? await renderPreviewScreenshot(iframe, target)
            : null;
          if (screenshot) {
        ...
        addInlineCommentDraft(
          { directory: effectiveDirectory, sessionKey },
          {
  ...
  React.useEffect(() => {
    ...
    const handler = (event: MessageEvent<PreviewBridgeMessage>) => {
      ...
      if (
        !data ||
        data.source !== "openchamber-preview-bridge" ||
        data.version !== 1
      ) {
        return;
      }
      ...
      if (data.type === "console") {
        ...
        const args = Array.isArray(data.args)
          ? data.args.map(stringify).filter(Boolean)
          : [];
        pushConsoleEvent({
          level,
          message: args.join(" "),
      ...
      if (data.type === "runtime-error") {
        ...
        pushConsoleEvent({
          ...
          message:
            ...
            t("contextPanel.preview.console.runtimeError"),
      ...
      if (data.type === "navigate-preview") {
        ...
        const navigation =
          data.navigation === "external" ? "external" : "proxy";
  ...
  const filteredConsoleEvents = consoleEvents.filter((event) =>
    getPreviewConsoleFilterMatch(event, consoleFilter),
  );
  ...
  const [upstreamState, setUpstreamState] =
    React.useState<UpstreamState>("unknown");
  ...
  return (
    <div className="absolute inset-0 flex flex-col">
      <div className="flex items-center gap-1 border-b border-border/40 bg-[var(--surface-background)] px-2 py-1">
        <div
          className="min-w-0 flex-1 truncate typography-micro text-muted-foreground"
        ...
        {isLoopback ? (
          <Button
            ...
            title={
              bridgeReady
                ? t("contextPanel.preview.console.open")
                : t("contextPanel.preview.console.waiting")
            }
            aria-label={
              bridgeReady
                ? t("contextPanel.preview.console.open")
          ...
          >
            ...
            {consoleErrorCount > 0 ? (
              <span className="typography-micro text-status-error">
                {consoleErrorCount}
              </span>
            ) : null}
          </Button>
          ...
          <div className="absolute inset-x-3 bottom-3 z-10 max-h-[45%] overflow-hidden rounded-xl border border-border/70 bg-[var(--surface-elevated)] shadow-lg">
            <div className="flex items-center justify-between border-b border-border/50 px-3 py-2">
              <div className="typography-ui-label text-foreground">
                {t("contextPanel.preview.console.title")}
              </div>
            ...
            <div className="flex items-center gap-1 border-b border-border/30 px-3 py-1.5">
              {(["all", "errors", "warnings", "logs"] as const).map(
                (filter) => (
                  <Button
                    key={filter}
                    type="button"
            ...
            <div className="max-h-64 overflow-auto p-2 typography-code text-xs">
              {consoleEvents.length === 0 ? (
                <div className="px-2 py-3 text-muted-foreground">
                  {t("contextPanel.preview.console.empty")}
                </div>
...
const IframeBrowserPane: React.FC<DesktopBrowserPaneProps> = ({
  initialUrl,
  directory,
  tabID,
}) => {
  const { t } = useI18n();
  const iframeRef = React.useRef<HTMLIFrameElement | null>(null);
  const setContextPanelTabTargetPath = useUIStore(
    (state) => state.setContextPanelTabTargetPath,
  );
  ...
  const [reloadNonce, bumpReload] = React.useReducer(
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        previewProxyTargetCache.set(proxyTargetKey, {
          proxyBasePath,
          previewToken,
          expiresAt,
        });
  ...
  const proxyUrlAuthKey =
    loadedUrl && proxyState.status === "ready"
      ? `${proxyState.proxyBasePath}|${proxyState.previewToken || ""}|${reloadNonce}`
      : "";
  ...
  const proxySrc = React.useMemo(() => {
    ...
    try {
      ...
      const search = searchParams.toString();
      return getRuntimeUrlResolver().authenticatedAsset(
        `${proxyState.proxyBasePath}${path}${search ? `?${search}` : ""}${parsed.hash}`,
      );
    } catch {
      return "";
    }
  ...
  const iframeSrc =
    proxySrc || (proxyState.status === "error" ? loadedUrl : "");
  ...
  const getCurrentUrlFromFrameUrl = React.useCallback(
    (frameUrl: string): string => {
      ...
      try {
        ...
        if (
          parsedFrameUrl.origin !== window.location.origin ||
          !parsedFrameUrl.pathname.startsWith(proxyBasePath)
        ) {
          return "";
        }
        ...
        return stripPreviewQueryParams(
          new URL(
            `${rest}${parsedFrameUrl.search}${parsedFrameUrl.hash}`,
            upstreamOrigin,
          ).toString(),
        );
      } catch {
  ...
  const getUpstreamUrlFromLocalFrameUrl = React.useCallback(
    (frameUrl: string): string => {
      ...
      try {
        ...
        if (
          parsedFrameUrl.origin !== window.location.origin ||
          upstreamOrigin === window.location.origin
        ) {
          return "";
        }
  ...
  const handleInspect = React.useCallback(() => {
    ...
    void (async () => {
      try {
        const target = await runIframeScript<unknown>(
          iframe,
...
const DesktopBrowserPane: React.FC<DesktopBrowserPaneProps> = ({
  ...
  React.useEffect(() => {
    ...
    const onNewWindow = (event: Event) => {
      const detail = (
        event as CustomEvent<{ url: string; disposition: string }>
      ).detail;
      if (
  ...
  React.useEffect(() => {
    ...
    const handler = (event: KeyboardEvent) => {
      ...
      try {
        webview
          ?.executeJavaScript?.(DESKTOP_BROWSER_CANCEL_INSPECT_SCRIPT)
          .catch(() => {});
      } catch {
        /* webview not ready */
      }
  ...
  const handleInspect = React.useCallback(() => {
    ...
    webview
      ...
      .then(async (target: unknown) => {
        ...
        const cssWidth = Number.isFinite(cssViewport?.width)
          ? (cssViewport as { width: number }).width
          : capture.width;
...
export const ContextPanel: React.FC = () => {
  ...
  const directoryKey = React.useMemo(
    () => normalizeDirectoryKey(effectiveDirectory),
    [effectiveDirectory],
  );
  ...
  const closeContextPanelTab = useUIStore(
  ...
  const setContextPanelWidth = useUIStore(
    (state) => state.setContextPanelWidth,
  );
  const setActiveContextPanelTab = useUIStore(
    (state) => state.setActiveContextPanelTab,
  );
  const reorderContextPanelTabs = useUIStore(
    (state) => state.reorderContextPanelTabs,
  );
  const setSelectedFilePath = useFilesViewTabsStore(
    (state) => state.setSelectedPath,
  );
  const openContextPreview = useUIStore((state) => state.openContextPreview);
  const createTerminalTab = useTerminalStore((state) => state.createTab);
  const contextEditorTreeVisible = useUIStore(
    (state) => state.contextEditorTreeVisible,
  );
  const toggleContextEditorTree = useUIStore(
    (state) => state.toggleContextEditorTree,
  );
  const allowPromptingSubagentSessions = useUIStore(
    (state) => state.allowPromptingSubagentSessions,
  );
  const { themeMode, setThemeMode, lightThemeId, darkThemeId, currentTheme } =
    useThemeSystem();
  ...
  const manualWidth = activeModeForWidth
    ? panelState?.widthByMode?.[activeModeForWidth]
    : undefined;
  const widthFraction = activeModeForWidth
    ? getContextSurfaceWidthFraction(activeModeForWidth)
    : 0.5;
  const widthFallbackBase =
    availablePanelAreaWidth ??
    (typeof window !== "undefined"
      ? window.innerWidth
      : CONTEXT_PANEL_DEFAULT_WIDTH * 2);
  ...
  const sessionTitleById = useSessionTitleMap(
  ...
  const chatFrameSrcByTabIDRef = React.useRef<
    Map<string, EmbeddedSessionChatURLCacheEntry>
  >(new Map());
  ...
  const resizeFollowTimerRef = React.useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  const clampWidthForDrag = React.useCallback((nextWidth: number) => {
    ...
    return available === null
      ? clamped
      : Math.min(clamped, Math.max(1, available));
  }, []);
  ...
  const handleResizeStart = React.useCallback(
    (event: React.PointerEvent) => {
      ...
      resizeAvailableWidthRef.current = getAvailablePanelWidth(
        panelRef.current,
      );
  ...
  React.useEffect(() => {
    ...
    const handleMove = (event: PointerEvent) => {
      ...
      if (resizeFollowTimerRef.current === null) {
        resizeFollowTimerRef.current = setTimeout(
          applyFollowWidth,
  ...
  const openResource = React.useCallback(
    (mode: "files-root" | "git" | "browser" | "terminal" | "extensions") => {
      ...
      const terminalTabId =
        mode === "terminal" ? createTerminalTab(directoryKey) : null;
  ...
  const resourceLauncher = React.useMemo(
    () => [
      {
        mode: "files-root" as const,
        icon: "folder-3" as const,
      ...
      },
      {
        ...
        label: t("contextPanel.mode.browser"),
      },
      {
        mode: "terminal" as const,
        icon: "terminal-box" as const,
        label: t("layout.mainTab.terminal"),
      },
      {
        mode: "extensions" as const,
        icon: "apps-2-ai" as const,
  ...
  const getEmbeddedChatSrc = React.useCallback(
    (tabID: string, sessionID: string, readOnly: boolean): string => {
      return getOrCreateEmbeddedSessionChatURL(
        chatFrameSrcByTabIDRef.current,
        tabID,
        sessionID,
        directoryKey || null,
        readOnly,
  ...
  const handleDiffScopeChange = React.useCallback(
    (nextScope: PendingDiffScope) => {
  ...
  const postChatSettingsSyncToEmbeddedChat = React.useCallback(() => {
    ...
    for (const frame of chatFrameRefs.current.values()) {
      ...
      frameWindow.postMessage(
        { type: "openchamber:chat-settings-sync", payload },
        window.location.origin,
      );
    }
      ...
      const directVisibilitySync = (
        frameWindow as unknown as {
          __openchamberSetEmbeddedVisibility?: (
            visibilityPayload: typeof payload,
          ) => void;
        }
      ).__openchamberSetEmbeddedVisibility;
  ...
  React.useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      const isKnownChatFrame = Array.from(chatFrameRefs.current.values()).some(
  ...
  const tabItems = React.useMemo(
    () =>
      activeModeTabs.map((tab) => {
        ...
        const tabPathLabel = getRelativePathLabel(
          tab.targetPath,
          effectiveDirectory,
        );
        return {
    ...
    <header className="flex h-10 items-stretch border-b border-border/40">
      ...
      <div className="flex items-center gap-1 px-1.5">
        ...
        <Button
          ...
          title={
            isExpanded
              ? t("contextPanel.actions.collapsePanel")
              : t("contextPanel.actions.expandPanel")
          }
          aria-label={
            isExpanded
              ? t("contextPanel.actions.collapsePanel")
  ...
  const panelStyle: React.CSSProperties = !isOpen
    ...
    : isExpanded
      ? {
          ...
          ["--oc-context-panel-width" as string]:
            availablePanelAreaWidth !== null
              ? `${availablePanelAreaWidth}px`
              : "100%",
          width:
            availablePanelAreaWidth !== null
              ? `${availablePanelAreaWidth}px`
  ...
  return (
    ...
    >
      ...
      {isOpen && !isExpanded && (
        <div
          aria-hidden="true"
          className="absolute left-0 top-0 z-40 h-full w-px bg-border/40"
        />
      )}
      ...
      {isOpen && (
        <div
          aria-hidden="true"
          className="absolute right-0 top-0 z-40 h-full w-px bg-border/40"
        />
      )}
      {!isExpanded && (
        <div
        ...
        >
          ...
          {tabs.some((tab) => tab.mode === "files-root") ? (
            <div
              className={cn(
                "absolute inset-0",
                isFilesRootActive ? "block" : "hidden",
              )}
            >
              <SidebarFilesTree />
            </div>
          ...
          {tabs.some((tab) => tab.mode === "extensions") ? (
            <div
              className={cn(
                "absolute inset-0",
                isExtensionsActive ? "block" : "hidden",
              )}
            >
              <ExtensionWorkbench />
            </div>
                ...
                {hasOpenEditorFile ? (
                  ...
                  <div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center">
                    ...
                    <div className="typography-ui-header text-foreground">
                      {t("contextPanel.editorEmpty.title")}
                    </div>
                    <div className="max-w-sm typography-micro text-muted-foreground">
                      {t("contextPanel.editorEmpty.description")}
                    </div>
                  </div>
                )}
              </div>
          ...
          {hasTerminalTab ? (
            <div
              className={cn(
                "absolute inset-0",
                activeTab?.mode === "terminal" ? "block" : "hidden",
              )}
            >
              <TerminalView
