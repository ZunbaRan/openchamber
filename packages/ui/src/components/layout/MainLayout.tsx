...
const DiagramView = lazyWithChunkRecovery(() =>
  import("@/components/views/DiagramView").then((m) => ({
    default: m.DiagramView,
  })),
);
...
const SettingsWindow = lazyWithChunkRecovery(() =>
  import("@/components/views/SettingsWindow").then((m) => ({
...
export const MainLayout: React.FC = () => {
  ...
  const isMultiRunLauncherOpen = useUIStore(
    (state) => state.isMultiRunLauncherOpen,
  );
  const setMultiRunLauncherOpen = useUIStore(
    (state) => state.setMultiRunLauncherOpen,
  );
  const multiRunLauncherPrefillPrompt = useUIStore(
    (state) => state.multiRunLauncherPrefillPrompt,
  );
  const isScheduledTasksPageOpen = useUIStore(
    (state) => state.isScheduledTasksDialogOpen,
  );
  const isArchivePageOpen = useUIStore((state) => state.isArchivePageOpen);
  const worktreesPageProjectId = useUIStore(
  ...
  const isSurfacePageOpen =
    isScheduledTasksPageOpen ||
    isArchivePageOpen ||
    Boolean(worktreesPageProjectId) ||
  ...
  React.useEffect(() => {
    ...
    const unsubscribeSession = useSessionUIStore.subscribe((state, prev) => {
      ...
      const draftOpened =
        ...
        state.newSessionDraft !== prev.newSessionDraft;
  ...
  const [mobileLeftDrawerVisible, setMobileLeftDrawerVisible] =
    React.useState(false);
  const [mobileRightDrawerVisible, setMobileRightDrawerVisible] =
    React.useState(false);
  ...
  const initialDrawerWidthRef = React.useRef(
  ...
  useEffect(() => {
    ...
    animate(
      rightDrawerX,
      mobileRightSidebarOpen ? 0 : rightDrawerWidth.current,
      {
        type: "spring",
        stiffness: 400,
        damping: 35,
  ...
  useEffect(() => {
    ...
    return leftDrawerX.on("change", (value) => {
      ...
      setMobileLeftDrawerVisible((previous) =>
  ...
  useEffect(() => {
    ...
    return rightDrawerX.on("change", (value) => {
      ...
      setMobileRightDrawerVisible((previous) =>
        previous === visible ? previous : visible,
      );
    });
  }, [isMobile, mobileRightSidebarOpen, rightDrawerX]);
    ...
    const scheduleDraftOpen = (delayMs: number) => {
      timeoutId = window.setTimeout(() => {
        ...
        if (
          ...
          sessionState.newSessionDraft?.open
        ) {
          return;
        }
  ...
  React.useEffect(() => {
    ...
    const handleResponsivePanels = () => {
      ...
      // viewport changes do not churn the panel layout.
      if (!isMobile && !isTablet) {
        ...
        if (shouldCloseRightSidebar) {
          ...
          canAutoOpenRightSidebar &&
          rightSidebarAutoClosedRef.current
        ) {
  ...
  }, [isMobile, isTablet, setRightSidebarOpen, setSidebarOpen]);
  ...
  const secondaryView = React.useMemo(() => {
    ...
    switch (activeMainTab) {
      ...
      case "git":
        return (
          <React.Suspense fallback={null}>
            <GitView isActive={!mobileRightSidebarOpen} />
          </React.Suspense>
        );
      case "diff":
        return (
            ...
            >
              ...
              >
                <div
                  className={cn(
                    "absolute inset-0",
                    (!isChatActive || isSurfacePageOpen) && "invisible",
                  )}
                >
                  <ErrorBoundary>
                    <ChatView
            ...
            >
              ...
              >
                <SessionSidebar
                  ...
                  onSessionSelected={handleDesktopSessionSelected}
                />
              </Sidebar>
              <div
                className="relative flex flex-1 min-w-0 flex-col overflow-hidden bg-background"
                data-page-scroll-lock="true"
              >
                <Header />
                <div
                ...
                >
                  <div
                    className="relative flex flex-1 min-w-0 flex-col overflow-hidden border-t border-border/50 bg-background"
                    data-page-scroll-lock="true"
                  >
                    <div
                      ...
                      >
                        ...
                        >
                          <div
                            className={cn(
                              "absolute inset-0",
                              (!isChatActive || isSurfacePageOpen) &&
                                "invisible",
                            )}
                          >
                            <ErrorBoundary>
