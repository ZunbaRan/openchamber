...
const MAX_KANBAN_COLUMNS = 24;
const MAX_KANBAN_CARDS_PER_COLUMN = 200;
...
export const TimelinePrimitive: React.FC<PrimitiveProps> = ({ node, resolveValue, emptyLabel }) => {
  const items = resolvedRecords(node.items ?? node.data, resolveValue).slice(0, MAX_TIMELINE_ITEMS);
  return (
...
export const ActivityFeedPrimitive: React.FC<PrimitiveProps> = ({ node, resolveValue, emptyLabel }) => {
  const items = resolvedRecords(node.items ?? node.data, resolveValue).slice(0, MAX_ACTIVITY_ITEMS);
  return (
    <section className={PANEL}>
...
export const HeatmapPrimitive: React.FC<PrimitiveProps> = ({ node, resolveValue, emptyLabel }) => {
  ...
  const rows = Array.from(new Set(cells.map((cell) => cell.row))).slice(0, MAX_HEATMAP_ROWS);
  const columns = Array.from(new Set(cells.map((cell) => cell.column))).slice(0, MAX_HEATMAP_COLUMNS);
...
export const KanbanPrimitive: React.FC<PrimitiveProps> = ({ node, resolveValue, emptyLabel }) => {
  const columns = resolvedRecords(node.columns, resolveValue)
    .filter((column) => typeof column.id === 'string')
    .slice(0, MAX_KANBAN_COLUMNS);
        ...
        <div className="grid min-w-max grid-flow-col auto-cols-[minmax(14rem,18rem)] gap-3">
          {columns.map((column) => {
            const columnCards = cards
              ...
              .slice(0, MAX_KANBAN_CARDS_PER_COLUMN);
            return (
