...
3. Declarative views render through `DeclarativeInteractiveView`; bindings are resolved by `lib/interactive-ui/bindings.ts` without `eval` or arbitrary expressions. The v1 set includes layout, metrics, text, progress, status, key-value, data-table, charts, lists, callouts, `flow`, `timeline`, `activity-feed`, `comparison`, `tabs`, `accordion`, `code-block`, `sparkline`, `gauge`, `heatmap`, `kanban`, `git-graph`, `tree`, `diff-summary`, and `divider`. A successful installed Declarative action refreshes declared queries. The explicit-only `interactive_ui_gallery` Tool renders the reviewed component Gallery in the normal conversation ToolPart flow.
...
7. `HTMLArtifactView` is a lazy chunk shared by Agent Generated and Third-party Extension Artifacts. Generated output is materialized from its exact Envelope; installed output resolves an already signed Artifact descriptor and verifies the originating Tool binding. Both load an authenticated outer Broker plus an opaque `data:` child and reuse the same iframe across inline/workspace/fullscreen.
...
- Scripted Artifact stop, runtime error, stalled heartbeat, and execution-lease expiry remove the active iframe and preserve a restart path. This is a bounded Host lifecycle control, not proof that every browser/runtime places the child in an independently killable OS renderer process; Scripts therefore remain experimental.
