...
import type { InstalledArtifactConfirmationHandle } from './InstalledArtifactConfirmationHost';

// Installed-only confirmation boundary: the confirmation hook and its popup
// live in this subordinate lazy chunk, rendered (and therefore loaded) only
// when the envelope is an installed Artifact. Generated and static Artifacts
// never load the confirmation chunk or instantiate its hook.
const InstalledArtifactConfirmationHost = React.lazy(
  () => import('./InstalledArtifactConfirmationHost')
    .then((module) => ({ default: module.InstalledArtifactConfirmationHost })),
);
...
export const HTMLArtifactView = React.forwardRef<HTMLArtifactViewHandle, HTMLArtifactViewProps>(({
  ...
  // Expanded state is computed from mode before the lazy confirmation host
  // so the child can portal the Host confirmation popup into this native
  // <dialog> only while it is in the top layer (workspace/fullscreen). Inline
  // and embedded workbench use the default body portal; passing undefined
  // keeps the hook's default for every existing caller.
  const expanded = !embeddedInWorkbench && mode !== 'inline';
  // Installed-only confirmation: the parent owns the lazy host's ref and
  // passes a stable fail-closed confirm into the business request path. A
  // missing ref (chunk still loading or not yet mounted) resolves false, so
  // the request cancels without retrying, writing, or queuing any options or
  // token.
  const installedConfirmationRef = React.useRef<InstalledArtifactConfirmationHandle>(null);
  const confirm = React.useCallback((options: InteractiveConfirmation) => {
    const host = installedConfirmationRef.current;
    return host ? host.confirm(options) : Promise.resolve(false);
  }, []);
    ...
    >
      ...
      {installed ? (
        <React.Suspense fallback={null}>
          <InstalledArtifactConfirmationHost
            ref={installedConfirmationRef}
            portalContainer={expanded ? hostRef : undefined}
            consumeEscape={expanded}
          />
        </React.Suspense>
      ) : null}
    </dialog>
