...
export const OpenChamberPlugin = async () => ({
  tool: {
    openchamber: {
      description: "Control OpenLoop projects, sessions, and scheduled tasks on the user's behalf. Sessions and scheduled tasks you create are for the user to follow and interact with; never use this tool to delegate parts of your own current task. Use one action per call. Scope with projectId or directory; omit both to use the current session directory. Session dispatches return immediately by default and you receive no notification when a dispatched session finishes, so never promise to report back on it; the user follows it in OpenLoop; a dispatched session needs no follow-up from you. If the user later asks how it went, use session.messages (add wait to block until it is idle, lastAssistant for just the final answer) — session.send always sends a NEW prompt and never just waits. Set wait only when the user asks or the next step requires the completed result. Session and worktree deletion are unavailable.",
      args: {
        action: { type: "string", enum: ${JSON.stringify(OPENCHAMBER_AGENT_TOOL_ACTIONS)}, oneOf: ${JSON.stringify(OPENCHAMBER_AGENT_TOOL_ACTION_DEFINITIONS.map(({ action, description }) => ({ const: action, description })))}, description: "OpenLoop action to perform" },
      ...
      async execute(input, context) {
        ...
        if (!endpoint || !token) {
          return failure({ schemaVersion: ${TOOL_SCHEMA_VERSION}, ok: false, action: args.action, error: { message: "OpenLoop managed tool connection is unavailable" } })
        }
        ...
        try {
          ...
          return failure({ schemaVersion: ${TOOL_SCHEMA_VERSION}, ok: false, action: args.action, error: { message: "OpenLoop returned an invalid response", kind: "runtime", status: response.status } })
        } catch (error) {
...
const mergePluginConfig = (rawConfig, pluginUrl) => {
  ...
  if (errors.length > 0 || !parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('OPENCODE_CONFIG_CONTENT must contain a valid JSON object before OpenLoop can inject its managed tool');
  }
  if (parsed.plugin !== undefined && !Array.isArray(parsed.plugin)) {
    throw new Error('OPENCODE_CONFIG_CONTENT plugin must be an array before OpenLoop can inject its managed tool');
  }
...
export const createAgentToolRuntime = (dependencies) => {
  ...
  const prepareManagedOpenCodeEnv = async () => {
    ...
    if (!Number.isInteger(port) || port <= 0) {
      throw new Error('OpenLoop listener port is unavailable for managed tool injection');
    }
  ...
  const execute = async (payload = {}, options = {}) => {
    ...
    if (!action || !ACTIONS.has(action)) {
      return createResult({ ok: false, action, error: { message: `Unsupported OpenLoop action: ${action || 'missing'}`, kind: 'usage' } });
    }
    if (typeof executeAction !== 'function') {
      return createResult({ ok: false, action, error: { message: 'OpenLoop control service is unavailable', kind: 'runtime' } });
    }
    try {
