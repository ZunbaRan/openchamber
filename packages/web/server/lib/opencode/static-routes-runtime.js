...
<head>
  ...
  <title>OpenLoop API-only mode</title>
  <style>
    :root {
      color-scheme: dark;
...
<body>
  <main>
    <svg class="logo" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="OpenLoop logo">
      <path d="M18.8 3.5C10.4 1.5 3.2 7.2 3.2 16.2C3.2 24.7 9.9 30.4 18 29.4C25.5 28.5 30.1 22.4 29.2 14.7C28.8 11.6 27.4 8.5 27 7C25.9 7.4 24.8 8.5 25.3 10.3C27.4 17.4 24.3 23.5 18 24.6C11.6 25.7 7.5 21.4 7.7 15.9C8 10.5 12.4 6.8 17.8 7.4C20.1 7.7 21.4 7 21.7 5.8C21.9 4.7 20.8 3.8 18.8 3.5Z" fill="currentColor" fill-opacity="0.2" stroke="currentColor" stroke-width="0.9"/>
    </svg>
    <h1>OpenLoop is running in headless mode</h1>
    <p>This server is ready. Open it from the OpenLoop desktop or mobile app to use it.</p>
    <div class="command">
  ...
  <script>
    ...
    button?.addEventListener('click', async () => {
      ...
      try {
        ...
        json: () => {
          res.json({ ok: true, mode: 'api-only', message: 'OpenLoop is running in API-only mode' });
        },
        default: () => {
          res.type('text/plain').send('OpenLoop is running in API-only mode');
        },
      });
    });
  };
