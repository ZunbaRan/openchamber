...
const createHarness = () => {
  ...
  registerOpenCodeRoutes(app, {
    ...
    getOpenCodeUpgradeCapability: () => ({ supported: false, reason: 'bundled' }),
    ...
    upsertProviderConfig: () => undefined,
...
describe('OpenCode distribution capability routes', () => {
  ...
  test('refuses independent OpenCode upgrades', async () => {
    ...
    expect(response.payload.code).toBe('OPENCODE_UPGRADE_MANAGED_BY_OPENCHAMBER');
    expect(response.payload.error).toContain('bundled with OpenChamber');
  });
});
