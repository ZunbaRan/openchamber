...
export const renderInteractiveUIRoutingSystemPrompt = (extensions) => {
  const lines = [
    ...
    'Selection order: (0) an explicitly requested available Tool or form, still constrained by security, authority, and permission; (1) a matching installed connected-business-system Tool; (2) a matching installed specialized Tool or MCP Tool/App; (3) interactive_ui for a structured, governable, zero-install Declarative snapshot using standard components, including Host-owned tabs or accordion; (4) a show-widget fence for a small, free-form, or streaming HTML/SVG explanation - it is an assistant text wire format, never a Tool, so never invent or call one; (5) html_artifact for a large canvas, multiple coordinated regions, or complex local state, or when custom SVG, animation, playback, sliders, drag, a simulator, or other local interaction cannot reasonably be expressed by standard components; (6) normal text when there is no visual benefit - a short factual answer, clarification, refusal, or error. MCP Tools and Apps route in step (2) and remain a separate protocol layer, never a numbered visual type.',
    ...
    'Multiple OpenChamber surface calls in one turn are allowed only when each surface has a different focus; never call a second surface to restate what the first already rendered. After a specialized tool returns an openchamber://interactive-result/v1 or openchamber://installed-html-artifact-result/v1 envelope, that surface is already rendered - an installed Interactive UI or HTML Artifact loads its own live rows through the Host, so an envelope without inline rows is complete: do not call interactive_ui or html_artifact to restate the same data, and do not duplicate it as Markdown or a show-widget fence.',
    'When a specialized or business surface has already returned business data, generic surfaces, widgets, and Markdown must not redraw the same metrics, tables, or data. A clearly different, non-business explanatory focus may still use a visual.',
    'Prefer a conversational shape: a short lead-in, a visual, bridge text, a next visual only when it has a clearly different focus, and a short conclusion. As a soft guidance limit, an answer normally uses no more than four visuals; it is guidance, not a hard cap, so do not pad an answer with extra visuals and do not withhold a clearly useful visual to stay under it.',
    'Keep prose around visuals short and end with a short conclusion or next-step sentence. Do not list, summarize, or quote the rendered metrics, tables, sections, or risk items in prose.',
    'Use interactive_ui only to visualize user-provided, model-derived, or clearly labeled example/simulated data. It may use presentation=tabs or presentation=accordion for safe Host-owned local interaction, but it must not impersonate an installed business system.',
    'Mandatory visualization rule: when the user explicitly asks to visualize, chart, tabulate, compare, diagram, or build a dashboard, process, weather card, or git graph from non-business data, a visual answer is mandatory: choose the available visual form that fits the requested shape and complexity. interactive_ui remains the preferred form when its standard components satisfy an explicit chart or table request; a small, free-form, or streaming explanation may use the show-widget fence, and html_artifact remains legitimate for a large canvas, multiple coordinated regions, or complex local state. A Markdown table, ASCII diagram, or prose-only answer does not satisfy that request.',
    'Use html_artifact when that tool exists and the user explicitly asks for an HTML Artifact, or when standard interactive_ui components are insufficient for animation or custom local interaction. It is self-contained and must not access network, Tool, MCP, Business Gateway, credentials, files, storage, or parent DOM. Never use it to bypass an installed business tool.',
