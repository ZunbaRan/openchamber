...
This module owns installed OCIX extension discovery, Interactive UI/HTML Artifact assets, and the Business Gateway.
...
The generated-Artifact capability response reports `scriptsMode: "experimental"` only when the explicit scripts gate is enabled; otherwise it reports `"unsupported"`. This gate does not control signed installed HTML Artifacts: those are package-reviewed surfaces, still isolated by the same opaque-origin sandbox and CSP.
...
- Every managed extension root is rechecked against the signed OCIX file-hash index before it is exposed to the runtime. A missing or mismatched integrity record fails closed and requires reinstall; an edited Native bundle, Declarative definition, HTML Artifact, or manifest is never loaded.
...
- CSP/sandbox declarations alone are not proof that a scripted Artifact emits no requests. Scripts therefore run in an opaque `data:` child behind the Broker, which relays only strict Bridge envelopes and removes the child after a navigation. The Chromium attack matrix observes form, frame, download, self-navigation, meta-refresh and API attacks while asserting zero actual target requests. The injected bootstrap now emits a lease-bound heartbeat, and the Host discards the iframe after a stalled heartbeat, a 15-minute execution lease, or an explicit user stop. Browser process placement is still not an independently killable OS boundary in every runtime, so Scripts remains experimental and default-off.
