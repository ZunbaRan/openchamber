...
- Choose only the components that fit the task: `metric-grid`, `chart`, `table`, `comparison`, `flow`, `timeline`, `activity-feed`, `gauge`, `heatmap`, `kanban`, `git-graph`, `tree`, `diff-summary`, `sparkline`, `progress`, `status`, `list`, `callout`, `code-block`, `divider`, and `text`.
