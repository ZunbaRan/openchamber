...
const createArtifactBrokerDocument = (artifactDocument) => {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>html,body{margin:0;width:100%;height:100%;overflow:hidden;background:transparent}iframe{display:block;width:100%;height:100%;border:0;background:transparent}</style></head><body data-ocix-artifact-broker><script>(()=>{"use strict";const frame=document.createElement("iframe"),pending=[];let channel="",loaded=false,loadCount=0,navigationBlocked=false;const size=value=>{try{return new TextEncoder().encode(JSON.stringify(value)).byteLength}catch{return Infinity}};const record=value=>value&&typeof value==="object"&&!Array.isArray(value);const keys=value=>Object.keys(value).every(key=>["source","direction","bridgeVersion","channelId","sequence","type","payload"].includes(key));const wheelKeys=value=>Object.keys(value).every(key=>["source","type","deltaX","deltaY","deltaMode","shiftKey","ctrlKey","altKey","metaKey"].includes(key));const validChannel=value=>typeof value==="string"&&value.length>=16&&value.length<=128;const hostMessage=value=>record(value)&&size(value)<=2200000&&keys(value)&&value.source==="openchamber-host"&&value.direction==="host-to-artifact"&&value.bridgeVersion===1&&["host.init","host.businessResult"].includes(value.type)&&validChannel(value.channelId);const artifactMessage=value=>record(value)&&size(value)<=65536&&keys(value)&&value.source==="openchamber-artifact"&&value.direction==="artifact-to-host"&&value.bridgeVersion===1&&value.channelId===channel;const wheelMessage=value=>record(value)&&size(value)<=1024&&wheelKeys(value)&&value.source==="openchamber-artifact-internal"&&value.type==="wheel-boundary"&&Number.isFinite(value.deltaX)&&Number.isFinite(value.deltaY)&&[0,1,2].includes(value.deltaMode);const reportNavigation=()=>{navigationBlocked=true;if(!channel)return;parent.postMessage({source:"openchamber-artifact-broker",direction:"broker-to-host",bridgeVersion:1,channelId:channel,type:"broker.navigationBlocked",payload:{}},"*")};addEventListener("message",event=>{const value=event.data;if(event.source===parent&&hostMessage(value)){if(value.type==="host.init")channel=value.channelId;else if(!channel||value.channelId!==channel)return;if(navigationBlocked){reportNavigation();return}if(loaded)frame.contentWindow?.postMessage(value,"*");else pending.push(value);return}if(event.source!==frame.contentWindow)return;if(artifactMessage(value)){parent.postMessage(value,"*");return}if(wheelMessage(value))window.postMessage({...value,source:"openchamber-artifact-broker-internal"},"*")});frame.title="Artifact content";frame.setAttribute("sandbox","allow-scripts");frame.src=${JSON.stringify(dataUrl)};frame.addEventListener("load",()=>{loadCount+=1;if(loadCount>1){frame.remove();reportNavigation();return}loaded=true;for(const message of pending.splice(0))frame.contentWindow?.postMessage(message,"*")});document.body.append(frame)})();</script></body></html>`;
};
