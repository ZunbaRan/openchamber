...
| Handing off or executing Generative Widget acceptance (unit gate, fixture UI, security, model E2E) | [Generative Widget test handoff](docs/GENERATIVE_WIDGET_TEST_HANDOFF.md) |
| Building a third-party Direct Remote service (signed Manifest URL + Access Key), or implementing/accepting the Remote path (R1–R4: connect, lazy resources, TTL cache, health/update, granted/re-consent, vendor references); not Host API foundation implementation | Start with the [Direct Remote service developer guide](docs/OCIX_REMOTE_SERVICE_DEVELOPER_GUIDE.md), then use the [OCIX Remote mode detailed plan](docs/OCIX_REMOTE_MODE_DETAILED_PLAN.md) and dual-path vision [OCIX_DUAL_PATH_HOSTED_AND_DEV_SERVER_PLAN.md](docs/OCIX_DUAL_PATH_HOSTED_AND_DEV_SERVER_PLAN.md) |
| OCIX self-dev Host-local backend (better-sqlite3 + Hono embedded Local Data Runtime), multi-app isolation, Agent app authoring rules | [OCIX Local Data Runtime plan](docs/OCIX_LOCAL_DATA_RUNTIME_PLAN.md) |
| OCIX Declarative / Trusted Native Style v2 implementation, maintenance, presets, layout/Kit contracts, or remaining visual acceptance | Start with [OCIX Style v2 agent brief](docs/OCIX_STYLE_V2_AGENT_BRIEF.md), then [Style contract](docs/OCIX_STYLE_CONTRACT.md), [detailed plan](docs/OCIX_DECLARATIVE_NATIVE_STYLE_V2_PLAN.md), and [development/acceptance report](docs/OCIX_STYLE_V2_DEV_REPORT_AND_TEST_PLAN.md) |
