...
- Pi attempts: none
- Primary attempts: not eligible while Pi retries remain
- Current evidence: Reopened 2026-08-10 by the real prepare gate. The downloaded arm64 archive matches the lock SHA-256, but its CLI does **not** contain the required Generative Widget assets, so `assertOpenCodeCliBinary` correctly rejects release `v1.18.10-oc.1`. The later clean local fork build at embedded commit `f263f908da3f71aa637ddb356328901b6ce231f2` passes the same asset self-check and is usable only through the explicit local-override path; it does not make the immutable GitHub Release lock true. The completed local installation proves that exact override is running from `/Applications/OpenChamber.app` as `1.18.10-oc.1+f263f908` with exact fork provenance, while the full packaged acceptance proves `interactive_ui` and `html_artifact`. This remains an immutable Release/publish blocker, but no longer blocks the completed P0 implementation/local-install milestone.
...
- Pi attempts: none
- Primary attempts: The first reviewer-approved installation was rolled back after issue085 exposed legacy Remote state. After issue085/086 corrections, a new fresh Sol/High review returned `ship`; primary then created and verified three timestamped backups, installed the exact final candidate, completed the real one-way migrations, and verified an idempotent second launch.
- Current evidence: Resolved 2026-08-11. Final HEAD `db5c10f5` passes the owning type/lint/unit/route gates, canonical arm64 package verification, dual-directory legacy-profile rehearsal, fresh Sol/High `ship`, and full packaged desktop acceptance (`ok:true`, native Runner ready/stop, 0 magenta leak pixels, 95.58% green guard ratio, restart persistence, `runtimeErrors:0`). `/Applications/OpenChamber.app` is now version 1.18.1 arm64, passes strict deep codesign, and retains final hashes app.asar `df3f6898…`, CLI `83b90c95…`, distribution `f0e72d15…`. The installed child process is `/Applications/OpenChamber.app/Contents/Resources/opencode-cli/opencode serve`, version `1.18.10-oc.1+f263f908`, repository `ZunbaRan/opencode`, fork commit `f263f908…31f2`. Backups are `/Applications/OpenChamber-1.17.1-backup-20260811-003613.app`, `/Users/loloru/.config/openchamber-backup-20260811-003613`, and `/Users/loloru/.config/opencode-backup-20260811-003613`; the original app is additionally retained at `/Applications/OpenChamber-1.17.1-preinstall-20260811-072525.app`. No user data or credentials were deleted.
- Continuation decision: Only a fully green and provenance-verified result permits P0 to return to done and P1 to resume.
- Next action: Resolved for local installation. Keep the immutable public Release/publish requirement tracked separately by issue066.
...
- Current evidence: Resolved 2026-08-11 as diagnostic-only/no-code. The issue081 rebuild preserved attach-before-load but initially timed out because the external Runner preload was absent (issue083), not because of opaque-origin sandboxing. The rejected same-origin relaxation remains reverted. Final package inspection and acceptance retain the stronger outer `sandbox allow-scripts` policy plus inner `sandbox="allow-scripts"`, reach native Runner ready/stop, prove clipping and restart persistence, and report `runtimeErrors:0`; the installed second launch is healthy. No supported behavior is observably affected by the remaining Chromium diagnostic.
- Suspension decision: n/a
- Resume condition: n/a
...
- Next action: Resolved with `result=not-triggered`; reopen only if a reproducible supported behavior fails under the retained strict sandbox.
...
## issue-085: Migrate verifiable pre-consent Remote state without blocking Desktop startup

- Status: RESOLVED
- Classification: P0
- Goal / user outcome: OpenChamber 1.18.1 starts against the existing 1.17.1 user profile without deleting extensions, credentials, settings, or other user data, while preserving the installation-scoped trust boundary for Direct Remote extensions.
- First-principles root cause: The installed 1.18.1 candidate reached the real packaged startup path, then `normalizeStoredRemoteMetadata()` rejected the existing `com.openchamber.reference.aurora-ops@1.0.0` record because its older durable shape has no `remote.publisherPublicKey`. The signed Manager-owned shell still contains the accepted hosted manifest and public key, but startup treats the missing newer private field as unrecoverable corruption before any bounded migration or quarantine decision. The launch closed immediately with `Remote identity is invalid`; the reviewer-approved package was preserved at `/Applications/OpenChamber-1.18.1-failed-20260810-222918.app`, and the verified 1.17.1 backup was restored to `/Applications/OpenChamber.app` without deleting user data.
- Core acceptance invariant: A narrowly recognized older Remote record may be migrated only when the existing Manager-owned shell, signed manifest, publisher tuple, manifest/package hash, connector binding, permissions, installation identity, and file hashes prove the same accepted installation. The migration must atomically establish every newer private authority record required after restart; malformed, substituted, incomplete, or unverifiable state remains fail-closed. One invalid extension must not terminate the whole Desktop process when it can instead be safely and visibly quarantined. No credential material may be read into logs, copied into Manager state, or discarded.
- Dependencies: issues 060, 063, 080, 070
- Dispatch order: Immediate serial blocker discovered by the installed-app smoke after the first fresh reviewer `ship`; fix and focused regression first, then rebuild, packaged acceptance, a new fresh reviewer, and a second recoverable installation attempt.
- Ownership: `packages/web/server/lib/interactive-ui/manager.js`; `packages/web/server/lib/interactive-ui/manager.test.js`; `packages/web/server/lib/interactive-ui/DOCUMENTATION.md` only if the accepted persisted-state contract changes; a narrowly scoped checked-in legacy fixture below `packages/web/server/lib/interactive-ui/fixtures/` only if it is generated from and byte-compared with the actual 1.17.1 writer. Primary owns `issues.md`, roadmap wording, host-profile verification, backup/replacement, and launch smoke.
- Focused verification: Add a fast deterministic Manager restart test using the exact older durable Remote shape (missing the newer installation-scoped key/consent state) and prove it fails before the fix. After the fix, prove exact migration or safe quarantine, atomic rollback on persistence failure, strict rejection of key/hash/identity substitution, preservation of unrelated extensions and secrets, idempotent second restart, and the existing Manager/Remote/feature-routes suites. Primary must then rebuild with the exact fork CLI override, rerun full packaged desktop acceptance, and launch against the real unchanged profile.
- Worker binding: shared-worktree session `/root/issue085_remote_state_migration`, stable issue ID `issue-085`, supervised local execution, no replacement issue.
- Worker attempts: Attempt 0 added a side-effecting `readState()` migration plus a synthetic current-state-minus-key fixture. Manager 102/102, Remote route/production wiring 25/25, Web typecheck/lint, syntax, and owned diff checks passed, but the independent security audit rejected the candidate before integration. Correction 1 kept `readState()` pure, serialized a startup gate, added the historical Tool/trust shape, required exact `remote-confirmation` authority, and changed to forward recovery; Manager reached 105/105, but primary inspection found two remaining security gaps. Correction 2 is the final worker correction: `remote-confirmation` must be readable only for migration and must never resolve as global Local-package trust, and a state-write failure must retain the exact non-authoritative consent residue for serialized forward retry rather than check-then-unlinking a path. No replacement worker or new issue is allowed for this root cause.
- Primary attempts: The first installation attempt was rolled back without deletion after the real profile produced a deterministic startup failure. After the accepted correction, primary proved the migration on an isolated real-profile copy, repeated it against the backed-up real profile, and verified an idempotent second installed launch.
- Current evidence: Resolved 2026-08-11. The final Correction 2 keeps reads pure, serializes one startup migration gate, verifies the exact historical Tool SHA and signed shell plus trust/identity/containment contracts, ignores `remote-confirmation` for every Local trust decision, publishes consent then state then retires only exact historical slots, and recovers forward without unlink/restore. Manager 107/107, host Remote routes/production wiring 25/25, Web typecheck/lint, dual-profile rehearsal, and fresh final review all pass. The real installed migration produced 3/3 Remote records with `publisherPublicKey`, 3 installation-scoped consents, and 0 legacy `remote-confirmation` slots; the connection-secret SHA-256 remains exactly `54c0e6bf…49df`, no transaction residue remains, and the second installed launch is healthy.
- Suspension decision: n/a
- Resume condition: n/a
- Continuation decision: Treat this as a concrete migration for an existing persisted consumer, not a general compatibility layer. Never repair from unsigned or external network data, and never make startup succeed by weakening Remote identity validation.
- Next action: Resolved; retain the timestamped OpenChamber/OpenCode configuration backups as rollback evidence.

## issue-086: Migrate exact legacy Agent Runtime ownership records

- Status: RESOLVED
- Classification: P0
- Goal / user outcome: OpenChamber 1.18.1 starts against the existing 1.17.1 Agent Runtime ownership inventory without deleting managed tools/skills or weakening ownership-conflict detection.
- First-principles root cause: After issue-085 successfully migrated the copied Remote installation/consent state, packaged startup reached Agent Runtime reconciliation and rejected the real 1.17.1 ownership record shape. Those schema-v1 records have one canonical record-level `versions` entry, while every `assets[]` descriptor contains `target`, `kind`, `name`, and `sha256` but no newer per-asset `version`; the current parser passes the missing value to `assertVersion()` and terminates startup with `Managed Agent Runtime version is invalid`.
- Core acceptance invariant: Recognize only the exact unambiguous legacy shape and historical writer bytes: schema/manager/extension identity valid, exactly one legal record version, every asset uniformly missing `version`, historical field/target ordering plus indentation/newline exact, authoritative `previousAssets` equal, and all existing managed-file content/hash, target/name/path/duplicate checks intact. Convert that shape to the current in-memory ownership contract and persist the current canonical record during reconciliation. Mixed old/new assets, multiple-version ambiguity, unknown fields, invalid paths/hashes, duplicate targets, nonhistorical formatting/reordering, and ordinary ownership conflicts remain fail-closed. Without an additional durable provenance marker, a byte-exact replay of the historical writer output that also matches authoritative state and managed files is information-theoretically indistinguishable and remains the explicit residual boundary. The migration must be idempotent and must not inspect or modify credential material.
- Dependencies: issues 070, 085
- Dispatch order: Immediate serial blocker found by the isolated real-profile launch after issue-085 state migration; fix/test first, then rebuild and repeat the isolated launch from a fresh copy.
- Ownership: `packages/web/server/lib/interactive-ui/agent-runtime.js`; its existing focused test file; `packages/web/server/lib/interactive-ui/DOCUMENTATION.md` only for the persisted migration contract. Primary owns this ledger, package rebuild, profile-clone verification, reviewer, backup, and installation.
- Focused verification: First reproduce the exact historical record as a failing test. Then prove canonical rewrite, idempotent second reconciliation, rejection of mixed/multiversion/unknown/duplicate/bad-path/bad-hash records, unchanged conflict detection, full Agent Runtime suite, relevant Manager/routes regressions, Web typecheck/lint, syntax, and diff checks. Primary must launch the rebuilt packaged app with both OpenChamber data and OpenCode config isolated from the real profile, confirm issue-085 and issue-086 migrations together, and verify original hashes remain unchanged.
- Worker binding: shared-worktree session `/root/issue086_agent_runtime_migration`; bounded ownership only, no install or real-profile access.
- Worker attempts: Attempt 0 added exact-shape parsing and canonical rewrite, with Agent Runtime 20/20 and all dependent gates green, but independent review found that any semantically valid legacy-shaped JSON bypassed byte-exact ownership-conflict detection and that one persistence assertion could pass vacuously. Correction 1 binds migration eligibility to the exact verified 1.17.1 writer bytes, strengthens nonempty/own-version persistence assertions, adds nonhistorical-format/reorder rejection, and documents the exact-replay residual boundary. Final independent correction review reports no actionable findings.
- Primary attempts: One isolated packaged launch against a copied OpenChamber data directory exposed the deterministic Agent Runtime failure after issue-085 had already changed only the copy's installation/trust state; original installation, trust, and connection-secret hashes remained unchanged.
- Current evidence: Resolved 2026-08-11. The accepted correction reproduces the historical writer's default version sort, `localeCompare` target order, field order, two-space JSON, and trailing newline solely as migration evidence; the current writer remains code-point ordered. Agent Runtime passes 21/21, Manager 107/107, Remote/production routes 25/25, Web typecheck/lint, dual-profile rehearsal, and two independent audit rounds. The real installed migration rewrote all 4/4 ownership records to nonempty canonical per-asset versions through the existing transaction, preserved every sampled managed Tool/Skill hash, left no transaction/temp residue, and the second installed launch was idempotent and started the bundled fork CLI.
- Suspension decision: n/a
- Resume condition: n/a
